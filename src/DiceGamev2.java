...
public class DiceGamev2 {
    ...
    public static void main(String[] args) {
        ...
        rules = printWelcomeMessage(rules);

        do {

            // Show how many games played for now (included this one)
            System.out.println("*********************");
            ...
            System.out.println("\nDo you wanna try another round and keep results over time? y/n");
    ...
    public static void printOverallWinStats(int[] flagArrayWinStat) {
        ...
        // Find out if player won with more or equal victories and assign flag
        boolean playerWins = (flagArrayWinStat[1] >= flagArrayWinStat[0]) ? true : false;
        ...
        System.out.println("*******                                         *******");
        System.out.printf("*******        TOTAL NUMBER OF GAMES % 2d         *******\n", flagArrayWinStat[3]);
        System.out.println("*******                                         *******");
        System.out.println("*********        THE WINNER IS " + ((playerWins) ? "PLAYER " : "MACHINE") + "        *********");
        System.out.printf("*********             WITH %d WINS             *********\n", (playerWins) ? (flagArrayWinStat[1]) : (flagArrayWinStat[0]));

        System.out.println("*********                                     *********");
        System.out.printf("********    %s LOOSES WITH ONLY %d WINS    ********\n", ((!playerWins) ? "PLAYER " : "MACHINE"), ((playerWins) ? flagArrayWinStat[0] : flagArrayWinStat[1]));
        System.out.println("*********                                    **********");
    ...
    public static int[] getRound(Scanner in, Random random, int[] flagArrayRoundTemp, int MACHINE_CASE, int[] flagArrayWinStat) {
        ...
        do {
            ...
            if (flagPlayer) {
                ...
                // inform the player of the number of throws
                System.out.println("*********************");
    ...
    public static int[] printWelcomeMessage(int[] rules) {
        System.out.println("*******************************************");
        System.out.println("***                                     ***");
        ...
        System.out.println("\nThese are the rules:");
        System.out.println("\nYour make a round of throws first and afterward the machine will compete");
        System.out.println("\nIf you go above 21 you loose");
        System.out.println("If you hit 21 you get Blackjack");


        // Print rules adjusted toward setting on possibility to match blackjack
        String blackjackRuleString = "\nIn case of Blackjack you automatically win";
        if (rules[1] == 1) {
            blackjackRuleString = blackjackRuleString.replace("you automatically win", "the machine can still win by throwing and reaching Blackjack");

        }
        System.out.println(blackjackRuleString);


        // print rules adjusted towards setting of who wins a draw
        System.out.println("\nIf the machine matches your throw it is a draw");
        String drawRuleString = "In case of a draw no one wins";
        if (rules[0] == 0) {
            drawRuleString = drawRuleString.replace("no one", "machine");
        }
        if (rules[0] == 1) {
            drawRuleString = drawRuleString.replace("no one", "player");
        }
        System.out.println(drawRuleString);


        // Inform the player that player wins if the total number of wins are equal
        System.out.println("\nIf the total number of wins are equal in the end, then the player will win");
